...
if ("storage" in navigator && "persisted" in navigator.storage) {
  navigator.storage.persisted().then((persisted) => {
    document.getElementById("persisted").innerHTML = persisted
      ? "persisted"
      : "not persisted";
  });
}
...
function readFiles(files) {
  ...
  for (var i = 0; i < files.length; ++i) {
    ...
    console.log("File is saved in Cache");
...
function nonPersistentNotification() {
  ...
  try {
    ...
    console.log("nonPersistentNotification sent");
  } catch (err) {
...
if ("permissions" in navigator) {
  ...
  function handleChange(permissionName, newState) {
    ...
    newStateInfo.innerHTML =
      "" +
      timeBadge +
      " State of " +
      permissionName +
      " permission status changed to " +
      newState +
      ".";
    ...
    try {
      navigator.permissions
        ...
        .then(function (permission) {
          document.getElementById(permissionName + "-status").innerHTML =
            permission.state;
